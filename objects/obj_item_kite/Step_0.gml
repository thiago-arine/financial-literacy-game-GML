...
if (instance_exists(_player_asset) && _can_interact) {
    ...
    if (can_collect && keyboard_check_pressed(vk_space)) {
        ...
        if (instance_exists(obj_inventory)) {
            // Tenta adicionar
            var _success = obj_inventory.inventory_add(spr_item_kite, 0, 1, "special", "Pipa");
            
            if (_success) {
                global.has_kite = true;
                instance_destroy();
            } else {
                show_debug_message("AVISO: inventory_add retornou FALSE (Matriz cheia?)");
            }
        } else {
            show_debug_message("ERRO CRÍTICO: obj_inventory não existe nesta Room!");
        }
    }
} else {
    can_collect = false;
